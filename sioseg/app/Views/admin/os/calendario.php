...
<div class="calendar-container">
    <header class="calendar-header">
        <div class="nav-section">
            <a href="<?= $rootUrl ?>/admin/os/calendario?year=<?= $nav['prev']['year'] ?>&month=<?= $nav['prev']['month'] ?>" class="nav-arrow" title="Mês anterior">
                <i class="fas fa-chevron-left"></i>
            </a>
            <h1 class="calendar-title"><?= htmlspecialchars($monthName) ?> de <?= htmlspecialchars($year) ?></h1>
            <a href="<?= $rootUrl ?>/admin/os/calendario?year=<?= $nav['next']['year'] ?>&month=<?= $nav['next']['month'] ?>" class="nav-arrow" title="Próximo mês">
