<div class="searcher-container">
    ...
    <form action="<?= BASE_URL ?>funcionario/os/search" method="GET">
                ...
                <div class="result-card">
                    <div class="card-header">
                        ...
                        <span class="status-badge status-<?= strtolower(str_replace(' ', '-', $os->status ?? '')) ?>">
                            <?= htmlspecialchars($os->status ?? 'N/A') ?>
                        </span>
                    </div>
                    <div class="card-body">
                        <p><i class="fa-solid fa-user"></i><strong>Cliente:</strong> <?= htmlspecialchars($os->nome_cli ?? '-') ?></p>
                        <p><i class="fa-solid fa-tools"></i><strong>Serviço:</strong> <?= htmlspecialchars($os->tipo_servico ?? '-') ?></p>
                        <p><i class="fa-solid fa-user-tie"></i><strong>Técnico:</strong> <?= htmlspecialchars($os->nome_tec ?? '-') ?></p>
                        <p><i class="fa-solid fa-calendar-alt"></i><strong>Agendamento:</strong> <?= !empty($os->data_agendamento) ? htmlspecialchars(date('d/m/Y H:i', strtotime($os->data_agendamento))) : '-' ?></p>
                    ...
                    <div class="card-footer">
                        <a href="<?= BASE_URL ?>funcionario/os/edit/<?= $os->id_os ?>">Ver Detalhes</a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
