document.addEventListener('DOMContentLoaded', function () { // Executa quando o DOM estiver carregado
    ...
    function toggleFields() { // Alterna campos baseado no tipo de pessoa selecionado
        ...
        if (tipoSelect.value === 'fisica') { // Se pessoa física selecionada
            ...
            document.getElementById('nome_cli').required = true;
            document.getElementById('cpf_cli').required = true;
            ...
            document.getElementById('nome_cli').required = false;
            document.getElementById('cpf_cli').required = false;
            ...
            var nomeField = document.getElementById('nome_cli');
            var cpfField = document.getElementById('cpf_cli');
            var razaoField = document.getElementById('razao_social');
            var cnpjField = document.getElementById('cnpj');
            if(nomeField) nomeField.required = false;
            if(cpfField) cpfField.required = false;
            if(razaoField) razaoField.required = false;
            if(cnpjField) cnpjField.required = false;
        }
        // Garantir que o CEP enviado ao servidor contenha apenas dígitos (remove máscara)
    ...
    function initializeFormScripts() { // Inicializa máscaras e validações dos campos
        ...
        var senhaInput = document.getElementById('senha_hash_cli'); // Campo senha
    ...
    function checkDuplicate(value, type, callback) {
        ...
        // Detecta se está na área do funcionário ou admin
        const currentPath = window.location.pathname;
        const isFuncionario = currentPath.includes('/funcionario/');
        const baseRoute = isFuncionario ? 'funcionario' : 'admin';
        
        const endpoint = window.BASE_URL + baseRoute + '/clientes/verificar-' + type;
